...
async def send_sms(
    ...
    try:
        ...
        if result["success"]:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"SMS 발송 실패: {result.get('error')}"
            )
...
async def send_verification(
    ...
    try:
        ...
        result = await notification_service.send_verification_sms(
            db=db,
            phone=phone,
...
async def verify_code(
    ...
    try:
        ...
        result = notification_service.verify_sms_code(
            db=db,
            phone=phone,
            code=request.code,
            purpose=request.purpose
