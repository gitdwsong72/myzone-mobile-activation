...
from app.core.file_permissions import require_file_write_permission, require_file_delete_permission
...
async def delete_file(
    filename: str,
    current_admin: Admin = Depends(get_current_admin),
    _: bool = Depends(lambda: require_file_delete_permission)
) -> Dict[str, str]:
    """
    파일 삭제
...
async def rescan_file_security(
    filename: str,
    current_admin: Admin = Depends(get_current_admin)
) -> Dict[str, Any]:
    """파일 보안 재검사 (관리자 전용)"""
