...
from ...core.gdpr import GDPRService, get_gdpr_service
from ...core.permissions import Permission, check_resource_ownership, require_user_permissions
...
async def get_my_order_detail(
    order_id: int, current_user: User = Depends(require_user_permissions(Permission.READ_ORDER)), db: Session = Depends(get_db)
):
    """내 주문 상세 조회"""
...
async def create_user(
    user_data: UserCreate, user_service: UserService = Depends(get_user_service), db: Session = Depends(get_db)
):
    """새 사용자 등록"""
    try:
        ...
        return {
            ...
            "data": {"id": user.id, "phone": user.phone, "email": user.email, "is_verified": user.is_verified},
        }
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="사용자 등록 중 오류가 발생했습니다.")
...
async def get_my_masked_profile(
    ...
    return {"success": True, "data": masked_info}


@router.post("/address/search", response_model=AddressSearchResponse)
async def search_address(search_request: AddressSearchRequest, user_service: UserService = Depends(get_user_service)):
    """주소 검색"""
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="주소 검색 중 오류가 발생했습니다.")
...
async def request_verification(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="본인인증 요청 중 오류가 발생했습니다.")
...
async def confirm_verification(
    ...
    """본인인증 확인"""
    try:
        success = user_service.confirm_verification(current_user.id, confirm_request.verification_id, confirm_request.code)

        if success:
            return {"success": True, "message": "본인인증이 완료되었습니다."}
        else:
            return {"success": False, "message": "본인인증에 실패했습니다."}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="본인인증 확인 중 오류가 발생했습니다.")
...
async def delete_my_account(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="계정 삭제 중 오류가 발생했습니다.")


@router.get("/me/data-export", response_model=Dict[str, Any])
async def export_my_data(
    current_user: User = Depends(get_current_user), gdpr_service: GDPRService = Depends(get_gdpr_service)
):
    """내 개인정보 내보내기 (GDPR Article 20)"""
    try:
        user_data = gdpr_service.export_user_data(current_user.id)
        return {"success": True, "message": "개인정보 내보내기가 완료되었습니다.", "data": user_data}
    except Exception as e:
        logger.error(f"개인정보 내보내기 실패: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="개인정보 내보내기 중 오류가 발생했습니다."
        )
...
async def get_privacy_processing_info(
    ...
    try:
        ...
        return {"success": True, "data": processing_info}
    except Exception as e:
        logger.error(f"개인정보 처리 현황 조회 실패: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="개인정보 처리 현황 조회 중 오류가 발생했습니다."
        )
...
async def anonymize_my_data(
    ...
    try:
        success = gdpr_service.anonymize_user_data(current_user.id, deletion_request.reason or "사용자 요청")

        if success:
            return {"success": True, "message": "개인정보가 익명화되었습니다."}
        else:
            return {"success": False, "message": "개인정보 익명화에 실패했습니다."}
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(f"개인정보 익명화 실패: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="개인정보 익명화 중 오류가 발생했습니다."
        )
