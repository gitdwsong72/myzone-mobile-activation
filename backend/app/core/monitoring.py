...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.endpoint_stats = defaultdict(
            lambda: {"count": 0, "total_time": 0, "avg_time": 0, "min_time": float("inf"), "max_time": 0, "error_count": 0}
        )
    ...
    def get_request_stats(self, minutes: int = 60) -> Dict[str, Any]:
        ...
        if not recent_metrics:
            return {"total_requests": 0, "avg_response_time": 0, "error_rate": 0, "requests_per_minute": 0}
    ...
    def _get_top_endpoints(self, metrics: List[RequestMetric], limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [{"endpoint": endpoint, "count": count} for endpoint, count in sorted_endpoints[:limit]]
...
class AlertManager:
    ...
    def check_alerts(self, metrics_collector: MetricsCollector):
        ...
        if request_stats["avg_response_time"] > self.alert_thresholds["response_time"]:
            self._send_alert(
                "high_response_time", f"High average response time: {request_stats['avg_response_time']:.2f}s", current_time
            )

        if request_stats["error_rate"] > self.alert_thresholds["error_rate"]:
            self._send_alert("high_error_rate", f"High error rate: {request_stats['error_rate']:.1f}%", current_time)

        # 시스템 관련 알림
        system_stats = metrics_collector.get_system_stats()

        if system_stats.get("cpu_percent", 0) > self.alert_thresholds["cpu_usage"]:
            self._send_alert("high_cpu_usage", f"High CPU usage: {system_stats['cpu_percent']:.1f}%", current_time)

        if system_stats.get("memory_percent", 0) > self.alert_thresholds["memory_usage"]:
            self._send_alert("high_memory_usage", f"High memory usage: {system_stats['memory_percent']:.1f}%", current_time)

        if system_stats.get("disk_percent", 0) > self.alert_thresholds["disk_usage"]:
            self._send_alert("high_disk_usage", f"High disk usage: {system_stats['disk_percent']:.1f}%", current_time)
    ...
    def _send_alert(self, alert_type: str, message: str, current_time: float):
        ...
        logger.critical(
            f"ALERT: {message}",
            extra={"alert_type": alert_type, "alert_message": message, "timestamp": datetime.now().isoformat()},
        )
