...
class HealthChecker:
    ...
    async def run_all_checks(self) -> SystemHealth:
        ...
        return SystemHealth(overall_status=overall_status, timestamp=datetime.now(), services=results, system_info=system_info)
    ...
    def _check_memory(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                service="memory", status=status, response_time=0.0, message=message, timestamp=datetime.now(), details=details
            )
    ...
    def _check_cpu(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                service="cpu", status=status, response_time=0.0, message=message, timestamp=datetime.now(), details=details
            )
...
class FailureRecovery:
    ...
    async def attempt_recovery(self, service: str) -> bool:
        ...
        try:
            ...
            # 복구 이력 기록
            self.recovery_history.append({"service": service, "timestamp": datetime.now(), "success": success})
            ...
            self.recovery_history.append({"service": service, "timestamp": datetime.now(), "success": False, "error": str(e)})
            return False
            ...
            try:
                ...
                logger.info(
                    ...
                    extra={
                        ...
                        "unhealthy_services": [s.service for s in health.services if s.status == HealthStatus.UNHEALTHY],
                    },
                )
