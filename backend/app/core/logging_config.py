...
def get_logging_config() -> Dict[str, Any]:
    ...
    config = {
        ...
        "formatters": {
            "structured": {"()": StructuredFormatter, "format": "%(timestamp)s %(level)s %(name)s %(message)s"},
        ...
        "loggers": {
            # 루트 로거
            "": {"level": "INFO", "handlers": ["console", "file_info", "file_error"], "propagate": False},
            # 애플리케이션 로거
            "app": {
            ...
            # 보안 로거
            "security": {"level": "WARNING", "handlers": ["console", "file_security"], "propagate": False},
            # 액세스 로거
            "access": {"level": "INFO", "handlers": ["file_access"], "propagate": False},
            # 외부 라이브러리 로거 레벨 조정
            "uvicorn": {"level": "INFO", "handlers": ["console"], "propagate": False},
            "uvicorn.access": {"level": "INFO", "handlers": ["file_access"], "propagate": False},
            "sqlalchemy": {"level": "WARNING", "handlers": ["console", "file_error"], "propagate": False},
...
def setup_logging():
    ...
    logger.info("Logging system initialized", extra={"environment": settings.ENVIRONMENT, "log_level": logger.level})
