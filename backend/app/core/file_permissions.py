...
class FilePermissionManager:
    ...
    def check_file_access_permission(
        self, file_path: str, user: Optional[User] = None, admin: Optional[Admin] = None, action: str = "read"
    ) -> bool:
        """파일 접근 권한 확인"""
        ...
        if user:
            ...
            permission_map = {"read": "can_download", "write": "can_upload", "delete": "can_delete"}
    ...
    async def _log_file_access(
        self, request: Request, file_path: str, action: str, user: Optional[User], admin: Optional[Admin]
    ):
        """파일 접근 로그 기록"""
        import logging
        ...
        logger.info(
            f"File access - User: {user_info}, Action: {action}, " f"File: {file_path}, IP: {client_ip}, UA: {user_agent}"
        )
...
async def require_file_read_permission(
    ...
    """파일 읽기 권한 필요"""
    return await file_permission_manager.validate_file_access(request, file_path, "read", db, user, admin)
...
async def require_file_write_permission(
    ...
    """파일 쓰기 권한 필요"""
    return await file_permission_manager.validate_file_access(request, file_path, "write", db, user, admin)
...
async def require_file_delete_permission(
    ...
    """파일 삭제 권한 필요"""
    return await file_permission_manager.validate_file_access(request, file_path, "delete", db, user, admin)
