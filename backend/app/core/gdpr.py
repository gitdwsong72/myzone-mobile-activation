...
from ..core.encryption import decrypt_personal_data, encryption_service
...
class GDPRService:
    ...
    def export_user_data(self, user_id: int) -> Dict[str, Any]:
        ...
        for order in orders:
            ...
            # 주문 상태 이력
            status_history = self.db.query(OrderStatusHistory).filter(OrderStatusHistory.order_id == order.id).all()
    ...
    def delete_user_data(self, user_id: int, reason: str = "사용자 요청") -> bool:
        ...
        try:
            ...
            self.db.query(OrderStatusHistory).filter(
                OrderStatusHistory.order_id.in_(self.db.query(Order.id).filter(Order.user_id == user_id))
            ).delete(synchronize_session=False)

            self.db.query(Payment).filter(
                Payment.order_id.in_(self.db.query(Order.id).filter(Order.user_id == user_id))
            ).delete(synchronize_session=False)
    ...
    def get_data_processing_info(self, user_id: int) -> Dict[str, Any]:
        ...
        return {
            "data_controller": {"name": "MyZone 통신서비스", "contact": "privacy@myzone.com", "dpo_contact": "dpo@myzone.com"},
            "processing_purposes": [
                "휴대폰 개통 서비스 제공",
                "고객 지원 및 문의 처리",
                "법적 의무 준수",
            ...
            "data_transfers": {"third_countries": "없음", "safeguards": "EU 적정성 결정 또는 표준 계약 조항"},
            "automated_decision_making": {"exists": False, "description": "자동화된 의사결정 없음"},
        }
