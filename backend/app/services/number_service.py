...
class NumberService:
    ...
    def get_numbers(
        ...
        numbers = (
            query
            .order_by(
                Number.is_premium.desc(),
                Number.additional_fee.asc(),
                Number.number.asc()
            )
            .offset((page - 1) * size)
            .limit(size)
    ...
    def get_available_numbers(self, category: Optional[str] = None, limit: int = 20) -> List[Number]:
        ...
        numbers = (
            query
            .order_by(Number.is_premium.desc(), Number.additional_fee.asc())
            .limit(100)  # 캐시용으로 더 많이 조회
            .all()
        )
    ...
    def analyze_number_pattern(self, number_str: str) -> NumberPatternAnalysis:
        ...
        # 특별한 숫자 조합 검사
        special_patterns = ['1234', '5678', '9876', '0000', '1111', '2222', '3333', '4444', '5555', '6666', '7777', '8888', '9999']
    ...
    def delete_number(self, number_id: int) -> bool:
        ...
        if number.status != "available":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="예약되었거나 사용 중인 번호는 삭제할 수 없습니다."
            )
    ...
    def _cleanup_expired_reservations(self):
        """만료된 예약 정리"""
        expired_numbers = (
            self.db.query(Number)
            .filter(
                and_(
                    Number.status == "reserved",
                    Number.reserved_until < datetime.utcnow()
                )
            )
            .all()
        )
