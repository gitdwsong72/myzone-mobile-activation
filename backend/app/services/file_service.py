...
class FileService:
    ...
    async def validate_file(self, file: UploadFile) -> bool:
        ...
        if mime_type not in self.ALLOWED_IMAGE_TYPES:
            raise HTTPException(
                status_code=400,
                detail=f"지원하지 않는 파일 형식입니다. 허용된 형식: {', '.join(self.ALLOWED_IMAGE_TYPES)}"
            )
        ...
        if not security_result['is_safe']:
            # 위험한 파일을 격리
            await file_security_service.quarantine_file(
                content, 
                file.filename, 
            ...
            raise HTTPException(
                status_code=400,
                detail=f"파일 보안 검사 실패: {'; '.join(security_result['issues'])}"
            )
    ...
    async def upload_device_image(self, file: UploadFile, device_id: int) -> dict:
        ...
        for size_name, image_data in variants.items():
            filename = self.generate_filename(
                file.filename, 
                f"device_{device_id}_{size_name}"
            )
