...
class EmailQueueService:
    ...
    async def _process_db_queue(self):
        ...
        pending_emails = (
            db.query(EmailQueue)
            .filter(EmailQueue.status == EmailStatus.PENDING, EmailQueue.scheduled_at <= datetime.utcnow())
            .limit(10)
            .all()
        )
    ...
    async def add_order_status_update_to_queue(
        self, to_email: str, user_name: str, order_number: str, status: str, note: Optional[str] = None, priority: int = 1
    ) -> bool:
        """주문 상태 변경 이메일을 큐에 추가"""
        status_names = {
            "confirmed": "접수 완료",
        ...
        return await self.add_to_queue(
            ...
            template_data={"user_name": user_name, "order_number": order_number, "status": status_display, "note": note},
            priority=priority,
        )
