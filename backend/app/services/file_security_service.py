...
class FileSecurityService:
    ...
    async def validate_file_security(self, file: UploadFile) -> Dict[str, any]:
        """파일 보안 검증"""
        validation_result = {"is_safe": True, "issues": [], "file_info": {}, "scan_results": {}}
    ...
    def _validate_image_file(self, content: bytes) -> Dict[str, any]:
        """이미지 파일 추가 검증"""
        result = {"is_valid": True, "issues": [], "metadata": {}}
    ...
    async def _scan_virus(self, content: bytes) -> Dict[str, any]:
        """바이러스 스캔 (ClamAV 사용)"""
        result = {"is_clean": True, "scanner": "none", "scan_result": "not_scanned"}
    ...
    def get_file_permissions(self, user_role: str, file_type: str) -> Dict[str, bool]:
        ...
        permissions = {"can_upload": False, "can_download": False, "can_delete": False, "can_view_metadata": False}

        if user_role == "admin":
            permissions.update({"can_upload": True, "can_download": True, "can_delete": True, "can_view_metadata": True})
        elif user_role == "manager":
            permissions.update({"can_upload": True, "can_download": True, "can_delete": False, "can_view_metadata": True})
        elif user_role == "user":
            if file_type in ["image", "document"]:
                permissions.update({"can_upload": True, "can_download": True, "can_delete": False, "can_view_metadata": False})
