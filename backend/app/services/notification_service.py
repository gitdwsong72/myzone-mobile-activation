...
class NotificationService:
    ...
    async def send_order_confirmation_notifications(
        ...
        logger.info(f"주문 확인 알림 발송 완료: order={order.order_number}, sms={results['sms']['success']}, email={results['email']['success']}")
    ...
    async def send_order_status_update_notifications(
        ...
        if new_status in sms_status_map:
            try:
                sms_result = await sms_service.send_order_status_sms(
                    phone=user.phone,
                    order_number=order.order_number,
                    status=sms_status_map[new_status]
                )
                results["sms"] = sms_result
            except Exception as e:
        ...
        logger.info(f"주문 상태 변경 알림 발송 완료: order={order.order_number}, status={new_status}, sms={results['sms']['success']}, email={results['email']['success']}")
    ...
    async def send_welcome_notifications(
        ...
        # 환영 SMS 발송
        try:
            sms_result = await sms_service.send_welcome_sms(
                phone=user.phone,
                name=user.name
            )
            results["sms"] = sms_result
        ...
        logger.info(f"환영 알림 발송 완료: user={user.name}, sms={results['sms']['success']}, email={results['email']['success']}")
