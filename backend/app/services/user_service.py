...
from ..schemas.user import (
    ...
    AddressSearchRequest, UserVerificationRequest
)
...
class UserService:
    ...
    def create_user(self, user_data: UserCreate) -> User:
        ...
        existing_user = self.db.query(User).filter(
            or_(User.phone == user_data.phone, User.email == user_data.email)
        ).first()
    ...
    def delete_user(self, user_id: int, reason: Optional[str] = None) -> bool:
        ...
        if active_orders > 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="진행 중인 주문이 있어 계정을 삭제할 수 없습니다."
            )
    ...
    def search_address(self, search_request: AddressSearchRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail="주소 검색 서비스에 일시적인 문제가 발생했습니다."
            )
