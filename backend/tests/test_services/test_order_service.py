...
from app.core.exceptions import OrderNotFoundError, InvalidOrderStatusError
...
class TestOrderService:
    ...
    def test_update_order_status_invalid_transition(self, db_session, created_user, created_plan, created_device, created_number):
        """잘못된 주문 상태 변경 테스트"""
        # Given
        order_service = OrderService(db_session)
        order = Order(
    ...
    def test_process_order_workflow(self, mock_notification, db_session, created_user, created_plan, created_device, created_number, created_admin):
        """주문 워크플로우 처리 테스트"""
        # Given
        order_service = OrderService(db_session)
        order = Order(
